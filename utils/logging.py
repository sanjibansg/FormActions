...
def remove_old(path):
    """Function for removing log files older than 7 days

    :param path: path where log files are saved
...
def logger(name):
    """Function for instantiating logger object

    :param name: name to be used for instantiation
    ...
    fileLog = logging.FileHandler(
        "logs/LOG_" + str(datetime.date.today()) + ".log", "a"
    )
